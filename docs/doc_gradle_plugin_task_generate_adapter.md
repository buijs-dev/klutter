...
Because the Android app depends on the local .aar file, transitive dependencies are not available. This means
any dependency of the .aar has to be added to the android/app build.gradle.

The generate adapters task will scan the build.gradle.kts file in the kmp/common (or other name, as specified
in the multiplatform DSL, see above) folder and copy all dependencies from the commonMain and androidMain blocks
in the sourcesets DSL. Right now <b>implementation</b> and <b>runtime only</b> dependencies are added to
the android/app build.gradle. Any variable used in the dependencies should be in the klutter.yaml file and 
have the appropriate camelcase name. 

Dependencies can be excluded by adding <b>//NOKLUTTER</b> at the end of the dependency line.

**Examples**

Given this property in klutter.yaml:
...
And this dependency in kmp/common build.gradle.kts:
...
kotlin {
    ...
    val kotlinxSerializationVersion = project.extra["kotlinxSerializationVersion"]

    sourceSets {
        val commonMain by getting {
            dependencies {
                implementation("dev.buijs.klutter:annotations-kmp:$klutterAnnotationsKmpVersion")//NOKLUTTER
...
Will add these dependencies in android/app build.gradle:
