...
class KlutterServicesDSL {
    fun configure(lambda: KlutterServiceBuilder.() -> Unit): KlutterServiceDTO {
        return KlutterServiceBuilder().apply(lambda).build()
    }
}

@KlutterServiceDSLMarker
class KlutterServiceBuilder {
...
class KlutterApiBuilder {
    ...
    fun func(name: String, async: Boolean = false, builder: KlutterApiFunctionBuilder.() -> Unit) {
        functionList.add(KlutterApiFunctionBuilder()
            .apply(builder).build()
